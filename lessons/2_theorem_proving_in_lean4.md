...
事实上，*Lean并不会保证你证明的结论在现实中是一定正确的*。Lean只能保证，**在某些给定的公理集合成立的前提下，你证明的某个定理是正确的**。

比如，“一个命题要么是真，要么是假” 这个听起来有道理的论断（被叫做[排中律](https://zh.wikipedia.org/wiki/%E6%8E%92%E4%B8%AD%E5%BE%8B)），所以我们把它当做 “经典逻辑系统” 的一部分。

然而，你也可以不承认排中律，构建其他的公理系统（比如直觉逻辑系统），你也可以得到一部分有趣的结论，并非所有的定理都依赖排中律。

> 在Lean中，排中律可以被 [选择公理](https://zh.wikipedia.org/wiki/%E9%80%89%E6%8B%A9%E5%85%AC%E7%90%86) 证明出来，从而是一个定理，但选择公理也有许多逻辑学家不承认，比如它可以导致[分球怪论](https://zh.wikipedia.org/wiki/%E5%B7%B4%E6%8B%BF%E8%B5%AB-%E5%A1%94%E6%96%AF%E5%9F%BA%E5%AE%9A%E7%90%86)。
...
* Lean中大部分时候不需要像传统程序那样编译并运行你的整个程序
    * 事实上在完成证明的时候，几乎主要都依靠交互式而运行整个程序来工作。
    * `#check`、`#eval`、`#print` 等指令提供了一种交互式输出信息（而非命令行输出）的方式。
...
> 
> 所以类型本身也有类型，而且类型的类型不可能是它自己（类型的类型和它自身不在同一宇宙）
> Sort 0包含的是Prop，Prop表示的是命题。Prop会在下一章去讲解。
...
返回一个命题的函数也是Prop，而这个函数的实现就是相应的证明：
...
另外，返回Prop类型的函数不是命题，它通常叫一个谓词，加入具体的参数后则可以作为一个命题，这个命题可能成立也可能不成立

* 例如下一章我们将在书中读到的 `def IsEven (a : Nat) := ∃ b, a = 2 * b` 
    * 它并不表示对于所有的 `a`, `∃ b, a = 2 * b` 都成立
    * 当代入具体的a之后，`IsEven a` 等价于 `∃ b, a = 2 * b` ，所以 `IsEven a` 构成一个命题
...
4. 对比 `#check (a: Nat)→a=1` 和 `#check (a: Nat)→Nat` 看看Message中有哪些区别？为什么会这样？猜一猜，`#check (a: Nat)→Prop` 是一个 `Prop` 还是一个 `Type` ？
...
* 存在量词 `∃ a:α, p` 
    * 在证明环境下可以直接解构它，得到一个使 `p` 成立的元素 `a` 和它成立的假设 `h`
    * 在非证明环境下不能直接解构它，但可以使用 `Exists.choose` 和 `Exists.choosespec` 替代，它们依赖 `Classical` 中的选择公理。
        * 实际上书中提到了，Exists可以看做一种信息擦除机制，所有的Exists命题是等价的，所以也就无法提取出构造时放进去的对象。
