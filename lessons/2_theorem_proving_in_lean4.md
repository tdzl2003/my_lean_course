...
* Prop是所有命题的类型
    ...
    * 如果一个命题存在实例，就意味着“命题为真”
    * 如果一个命题作为一个参数，意味着“如果命题为真，则可以blahblah”
    * True是一个Prop，它只有一个取值（代表着它成立），可以被实例化。所以True命题是真的。
...
* 合取同时是一个结构体，两个成员分别表示两个子命题成立的事实
    * 如果你拿到了合取成立的事实，就可以得到两个子命题各自成立的事实
    * 如果你想要得到一个合取成立的事实，就需要分别提供两个子命题成立的事实
* 析取同时是一个枚举类型，两种枚举情况分别表示两个子命题成立的事实。
    * 如果你拿到了析取成立的事实，你可以分别讨论两个子命题各自成立的情况，在两种情况均给出你的证明目标，就可以证明你的目标结论。
    * 如果你想要得到一个析取成立的事实，你只需要提供它任意一个自命题成立的事实。

### 补充练习

* 对比 `#check 1=2` 和 `#check 1==2` ，并试一下把 `#check` 改成 `#eval`
* 试着在`Bool`的运算中使用`Prop`，比如 `#check (1=2) || (3=4)` 看看它变成了啥类型。
* 想一下，如果要证明一个命题对另一个命题独立，该怎么定义？（命题独立 也应该是一个命题）
    * 具体的，怎么定义整数a和b的两个命题  `b>0` 对 `a>0` 独立？（只需要给出定义，无需给出证明）
        * 注意整数是`Int`类型而不是`Nat`
        * 可以用 := sorry 来跳过证明并获得一个警告
    * 看看你的定义的定理，用“对于”、“如果”的表述翻译一遍，看看是否正确？
* 对比 `#check (a: Nat)→Prop` 和 `#check (a: Nat)→Nat` 看看Message中有哪些区别？为什么会这样？猜一猜，`#check (a: Nat)→a=1` 是一个 `Prop` 还是一个 `Type` ？
...
* 想一想，`∃ a:α, False` 表示了什么？ `∀ a:α, False` 呢？它们有什么区别？
* 尝试用 `calc` 化简计算并证明：`example : 5 * x - 3 * x = 2 * x`
* `calc` 也可以用于逻辑等价，尝试用 `calc` 变形并求解方程 `example : ∀ x:ℝ, x+3=5 ↔ x=2`
